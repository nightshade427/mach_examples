...
void create_todo(){
  ...
  db_query_async_await("todos", "todos_db", q, uid, t);
  ds_render_async(client, mode_prepend, "todos", "todo");
}
...
void update_todo(){
  ...
  auto const uid = get("user_id");
  auto const todo = sfmt("todo_/%s", tid);
  auto const client = sfmt("todos/%s", uid);
  db_find_async_await("todos", "todos_db", q, f, uid, tid);
  ds_render_async(client, mode_replace, todo, "todo");
}
...
void delete_todo(){
  ...
  auto const todo = sfmt("todo_/%s", tid);
  auto const client = sfmt("todos/%s", uid);
  db_find_async_await("todos", "todos_db", q, uid, tid);
  ds_render_async(client, mode_remove, todo, nullptr);
}
