...
* **Built-in Tooling:** ai assistance with full application context, ~20ms hmr from save to browser, apm, log aggregation, error reporting, and debugging.
...
Handlers push events to a channel using the sse response function.

```c
auto const client = format("todos/{{user_id}}");

respond_sse((sse){.client = client, .data = {"some data"}});                          // send a data event
respond_sse((sse){.client = client, .event = "update", .data = {"some event"}});      // send a named event
respond_sse((sse){.client = client, .comment = "keepalive"});                         // send a comment
```

When `.client` is set, `respond_sse` pushes to the named channel, delivering the event to all connected clients on that channel. When `.client` is not set, `respond_sse` sends the event directly as the response to the current request.

```c
// push to all connected clients on the channel
respond_sse((sse){.client = client, .data = {"new todo added"}});

// respond directly to the current request
respond_sse((sse){.data = {"welcome"}});
```
...
When `.client` is not set, `respond_datastar` responds directly to the current request. When `.client` is set, it pushes to a named sse channel, allowing any handler or event subscriber to update any connected client.
...
// respond to current request
respond_datastar((ds){
  .js = "console.log('hello from server')"
});
...
The ai has full access to the running application through `app_info`, including routes, middleware, events, databases, schemas, and module topology. This means the ai generates code that fits your specific application, not generic boilerplate.
...
| `q` | `.db`, `.resource`, `.query` | Database query. `.db` is the database name. Use `.resource` for a named sql resource or `.query` for inline sql. |
| `sse` | `.client`, `.event`, `.data`, `.comment` | SSE event. `.client` is the sse channel name, when not set responds directly to the current request. `.event` is the event type, `.data` is an array of data lines, `.comment` is an sse comment. |
...
| Function | Description |
|---|---|
| `respond_sse(sse)` | Sends an sse event. The `sse` struct supports `.client` (sse channel name, when not set responds directly to the current request), `.event` (named event type), `.data` (array of data lines), and `.comment`. |
...
// send a named event to a channel with multiple data lines
respond_sse((sse){
  .client = client,
  .event = "todo_created",
  .data = {
...
| Type | Values |
|---|---|
| `datastar_mode` | `mode_outer`, `mode_inner`, `mode_replace`, `mode_prepend`, `mode_append`, `mode_before`, `mode_after`, `mode_remove` |

| Type | Fields | Description |
|---|---|---|
| `ds` | `.client`, `.mode`, `.target`, `.elements`, `.signals`, `.js` | Datastar response. `.client` is the sse channel name, when not set responds directly to the current request. `.mode` is the merge mode, `.target` is the dom element, `.elements` is a `b` body with template content, `.signals` is a JSON string to update client signals, `.js` is JavaScript to execute on the client. |
...
| `respond_datastar(options)` | Sends a Datastar response using the `ds` struct. |
