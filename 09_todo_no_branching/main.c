...
ma_state render_todos(const ma_ctx ctx){
  ma_render(ctx, ma_ok, m_todos);
  return ma_complete;
}

ma_state render_todo(const ma_ctx ctx){
  if(is_htmx(ctx)){
...
ma_state list_todos(const ma_ctx ctx){
  ...
  ma_db_query(ctx, render_todos, m_todos, m_todos, q, uid);
...
ma_state create_todo(const ma_ctx ctx){
  ...
  ma_db_query(ctx, render_todo, m_todo, m_todos, q, uid, t);
  return ma_processing;
}
...
ma_state update_todo(const ma_ctx ctx){
  ...
  ma_db_find(ctx, render_todo, m_todo, m_todos, q, f, uid, tid);
  return ma_processing;
}
...
ma_state delete_todo(const ma_ctx ctx){
  ...
  auto const uid = ma_cookies_get(ctx, m_user_id);
  auto const tid = ma_params_get(ctx, "id");
  ma_db_find(ctx, nullptr, m_todo, m_todos, q, uid, tid);
  return ma_complete;
}
